...
namespace WebApi
{
    ...
    {        
        ...
        /// <returns></returns>
        public static string CreateTokenByHandler()
        {
            var now = DateTime.UtcNow;
            Dictionary<string, object> payLoad = new Dictionary<string, object>();
            payLoad["nbf"] = ToUnixEpochDate(now);//可用时间起始
            payLoad["exp"] = ToUnixEpochDate(now.Add(TimeSpan.FromMinutes(60)));//可用时间结束            
            ...
            var jwt = new JwtSecurityToken(
                ...
                signingCredentials: new SigningCredentials(new SymmetricSecurityKey(Encoding.ASCII.GetBytes(ConfigHelper.GetValue("SecurityKey"))), SecurityAlgorithms.HmacSha256));
        ...
        public static bool Validate(string encodeJwt, Func<Dictionary<string, object>, bool> validatePayLoad)
        {
            ...
            var hs256 = new HMACSHA256(Encoding.ASCII.GetBytes(ConfigHelper.GetValue("SecurityKey")));
            //首先验证签名是否正确（必须的）
